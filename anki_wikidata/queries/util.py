...
def run_query(query: str, /, *, delay: int = 4, cache: Optional[Path] = None) -> str:
    ...
    sleep(delay)
    result = get(query_url(query), params={"format": "json"}).text
...
def get_results(
    query: str, /, *, delay: int = 4, cache: Optional[Path] = Path(".cache")
) -> list[dict[str, str]]:
